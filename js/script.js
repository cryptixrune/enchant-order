    ...
    for (var j = 0; j < group.length; j++) {
      ...
      for (var k = 1; k <= max_max; k++) {
        if (max >= k) {
          row.append(
            $("<td>").append(
              $("<button>").append(k).addClass("off").data({
                level: k,
                enchant: enchant,
              })
            )
...
function completed(msg) {
  ...
  if (elapsed <= 1) {
    $("#timings").text(
      "Completed in under a second, trying " +
        number_format(total_tries) +
        " combinations"
    );
  } else {
    $("#timings").text(
  ...
  for (var i = 0; i < msg.path.length; i++) {
    ...
    $("#steps").append(
      $("<li>").html("Combine <i>" + left + "</i> with <i>" + right + "</i>")
    );
  }
...
function expand_name(k, names, context) {
  ...
  if (bits.length == 1) {
    ...
    return (
      '<img src="./images/' + icon + '.gif" class="icon"> ' + names[bits[0]]
    );
  }
...
function button_clicked(elm) {
  ...
  if (elm.attr("class") == "on") {
    ...
    $("#enchants button")
      .filter(function () {
        return (
          $(this).data("enchant") == b_data.enchant &&
          $(this).data("level") != b_data.level
        );
      })
      .attr("class", "off");
    ...
    if (b_data.enchant == "Channeling" || b_data.enchant == "Loyalty") {
      $("#enchants button")
        .filter(function () {
          return $(this).data("enchant") == "Riptide";
        })
        .attr("class", "off");
      ...
      $("#enchants button")
        .filter(function () {
          return (
            $(this).data("enchant") == "Channeling" ||
            $(this).data("enchant") == "Loyalty"
          );
        })
        .attr("class", "off");
    } else {
      ...
      for (var i = 0; i < e_info.incompatible.length; i++) {
        $("#enchants button")
          .filter(function () {
            return $(this).data("enchant") == e_info.incompatible[i];
          })
          .attr("class", "off");
      }
    }
